...
const EditItinerary = ({ dayItinerary, onClose }: EditItineraryProps) => {
  const [itinerary, setItinerary] = useState<ItineraryType>({
    _id: dayItinerary._id,
    day: dayItinerary.day,
    title: dayItinerary.title,
    description: dayItinerary.description,
    accommodation: dayItinerary.accommodation || [],
    fineDining: dayItinerary.fineDining || [],
    note: dayItinerary.note,
    links: dayItinerary.links || [],
    itineraryDayPhoto: dayItinerary.itineraryDayPhoto,
    itineraryDayPhotoPreview: dayItinerary.itineraryDayPhotoPreview,
  })
    ...
    setItinerary((prev) => {
      ...
      if (currentAccommodations.includes(accommodationId)) {
        ...
        if (currentAccommodations.length < 3) {
          ...
          toast.error("Maximum 3 accommodations allowed per itinerary day")
          return prev
        }
      }
    })
    ...
    setItinerary((prev) => {
      ...
      if (currentFineDining.includes(fineDiningId)) {
        ...
        if (currentFineDining.length < 3) {
          ...
          toast.error("Maximum 3 fine dining allowed per itinerary day")
          return prev
        }
      }
    })
  ...
  const handleUpdateItinerary = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL_PROD}/itinerary/edit/${itinerary._id}`,
        formData,
        {
          headers: {
      ...
      toast.success(response.data.message || "Itinerary updated successfully!")

      // Close the edit form after successful update
      onClose()
    } catch (error: any) {
      toast.error(error?.response?.data?.message || "Error updating itinerary.")
  ...
  const handleDeleteItinerary = async (id: string) => {
    if (!id) {
      toast.error("Invalid itinerary ID.")
      return
    }
    ...
    try {
      ...
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL_PROD}/itinerary/delete/${id}`
      )
      toast.success(response.data.message || "Itinerary deleted successfully!")
      // Close the edit form after successful deletion
      onClose()
    } catch (error: any) {
      toast.error(error?.response?.data?.message || "Error deleting itinerary.")
  ...
  return (
    <div className="w-full mx-auto p-6">
      ...
      <div className="flex justify-end mt-4">
        ...
        >
          {isDeleting ? "Deleting Itinerary..." : "Delete Itinerary"}
        </Button>
      </div>
    </div>
  )
