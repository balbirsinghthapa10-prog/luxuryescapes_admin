...
const MailSendTailorMade: React.FC<MailSendFormProps> = ({
  email,
  name,
  onChange,
  id,
  ...
  const handleSendEmail = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL_PROD}/tailor-made/reply-to-client`,
        formData,
        {
          headers: {
  ...
  return (
    <div className="w-[400px] mx-auto p-6 bg-white shadow-md rounded-lg">
      <h1 className="mb-4 font-semibold">Reply,</h1>
      <form onSubmit={handleSendEmail}>
        <div className="mb-4 flex gap-4">
          <label className="block text-gray-700 mb-2">To:</label>
          <p className="text-blue-800 font-semibold">{email || "Loading..."}</p>
        </div>
        <div className="mb-4">
...
export default MailSendTailorMade
